# -*- coding: utf-8 -*-
"""
Created on Wed Nov  7 09:04:35 2018
...
def trainModel(xval,yval,b,wval,eta,rows):
...
def labelPredict(xeval,yeval,bestWeight,bestBias,rowsEval):
    ...
    fopen = open('../movie-ratings/results/decayingPerceptron-results.txt', 'w');
    ...
    for eta in etas:
        ...
        for trainSet in cvTrainx:
            ...
            for epoch in range(0,10):
                ...
                eta = eta/(1+epoch);
                wNew,biasNew,updates = trainModel(cvtrx,cvtry,biasNew,wNew,eta,rowtr); 
               # wNew,biasNew,updates = trainModel(trainSet,cvTrainy[cvCount],biasNew,wNew,eta,rows[cvCount]);
               
            del cvtrx,cvtry,rowtr;   
            #After ten epoch calculate the accuracy
            #accuracy = accuracyCalc(cvTestx[cvCount],cvTesty[cvCount],wNew,biasNew,rows[cvCount]);
            cvttx,cvtty,rowte = getParamsTest(cvCount);
            accuracy = accuracyCalc(cvttx,cvtty,wNew,biasNew,rowte); print("accuracy:",accuracy);
...
def main():
    print("Perceptron Model - Decaying learning rate");
    print("=========================================");
    ...
    #devFile = open('../dataset/diabetes.dev',"r",encoding="utf8");
    ...
    for epoch in range(0,20):
        #etaN = eta;
        eta = eta/(1+epoch);
        wNew,biasNew,updates = trainModel(x,y,biasNew,wNew,eta,rows);
        ...
        accuracy = accuracyCalc(xdev,ydev,wNew,bias,rowsDev);
